...
contract AtomicArbHookScript is Script {
    ...
    function run() public {
        ...
        (address hookAddress, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, permissions, type(AtomicArbHook).creationCode, abi.encode(address(manager))
        );
        ...
        // ----------------------------- //
        vm.broadcast();
        AtomicArbHook atomicArbHook = new AtomicArbHook{salt: salt}(manager);
        require(address(atomicArbHook) == hookAddress, "AtomicArbHookScript: hook address mismatch");
        ...
        vm.startBroadcast();
        testLifecycle(manager, address(atomicArbHook), lpRouter, swapRouter);
        vm.stopBroadcast();
    }
